...
final journalFilterProvider = StateProvider<JournalFilter>(
  (ref) => const JournalFilter(),
);

final journalListProvider = FutureProvider.autoDispose<List<HealthRecord>>((
  ref,
) async {
  final repo = ref.watch(journalRepoProvider);
  final filter = ref.watch(journalFilterProvider);
  final results = await repo.fetchAll(
    type: filter.type,
    from: filter.from,
    to: filter.to,
    noteQuery: filter.query,
  );
  return results;
});

final selectedRecordProvider = FutureProvider.family<HealthRecord?, int>((
  ref,
  id,
) async {
  final repo = ref.watch(journalRepoProvider);
...
final last7DaysProvider =
    FutureProvider.autoDispose<Map<HealthMetricType, List<HealthRecord>>>((
      ref,
    ) async {
      final repo = ref.watch(journalRepoProvider);
      ...
      for (final type in HealthMetricType.values) {
        final rows = await repo.fetchAll(type: type, from: from, to: now);
        result[type] = rows;
      }
      return result;
    });
